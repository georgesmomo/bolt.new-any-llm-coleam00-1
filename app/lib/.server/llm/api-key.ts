...
export function getAPIKey(cloudflareEnv: Env, provider: string): string {
  /**
   * The `cloudflareEnv` is used when deployed or when previewing locally.
   * In development, the environment variables are available through `process.env`.
   * Adjusting this function to be compatible with Vercel.
   */
  switch (provider) {
    case 'Anthropic':
...
export function getBaseURL(cloudflareEnv: Env, provider: string): string {
  /**
   * Retrieves the base URL for specific providers.
   * Uses `cloudflareEnv` for deployed environments and `process.env` for local development.
   */
  switch (provider) {
    case 'OpenAILike':
